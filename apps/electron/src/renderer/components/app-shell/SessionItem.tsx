      ...
      icon={
        <>
          ...
          <div className={cn(
            ...
            (item.isProcessing || hasUnreadMeta(item) || item.lastMessageRole === 'plan' || item.lastMessageRole === 'error' || hasPendingPrompt)
              ? "opacity-100 ml-0"
            ...
            {item.lastMessageRole === 'error' && (
              <CircleAlert className="h-3.5 w-3.5 text-destructive" aria-label="Session ended with an error" />
            )}
