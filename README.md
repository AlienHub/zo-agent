...
## How it Works (Video)
To understand what Zo does and how it works watch this video.
...
## Why Zo was built
Zo is a fork of Craft Agents: a tool built so teams can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
Zo is open source under the Apache 2.0 license - so you are free to remix and change anything.

This fork continues that direction under the Zo name.
...
Zo connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
Zo can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
Zo supports multiple ways to connect to LLM providers:
...
**macOS:**
```bash
/Applications/Zo.app/Contents/MacOS/Zo -- --debug
```

**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\Zo\Zo.exe" -- --debug
```
