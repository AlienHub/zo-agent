...
function getExistingGlobalSlugs(): Set<string> {
  ...
  mkdirSync(getWorkspaceSkillsPath(emptyWs), { recursive: true });
  try {
...
describe('loadSkill', () => {
  ...
  it('should return null for directory without SKILL.md', () => {
    createEmptySkillDir(getWorkspaceSkillsPath(workspaceRoot), 'empty-skill');
  ...
  it('should return null for invalid SKILL.md (missing required fields)', () => {
    createInvalidSkill(getWorkspaceSkillsPath(workspaceRoot), 'bad-skill');
  ...
  it('should load skill with optional globs', () => {
    createSkill(getWorkspaceSkillsPath(workspaceRoot), 'frontend', {
      globs: ['*.tsx', '*.css'],
    });
  ...
  it('should load skill with normalized requiredSources', () => {
    createSkill(getWorkspaceSkillsPath(workspaceRoot), 'with-sources', {
  ...
  it('should normalize single-string requiredSources into an array', () => {
    const skillDir = join(getWorkspaceSkillsPath(workspaceRoot), 'single-source');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), `---
name: "Single Source"
...
`);
  ...
  it('should ignore invalid requiredSources entries', () => {
    const skillDir = join(getWorkspaceSkillsPath(workspaceRoot), 'invalid-sources');
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, 'SKILL.md'), `---
...
`);
  ...
  it('should set iconPath when icon file exists', () => {
    const skillDir = createSkill(getWorkspaceSkillsPath(workspaceRoot), 'with-icon');
  ...
  it('should not set iconPath when no icon file exists', () => {
    createSkill(getWorkspaceSkillsPath(workspaceRoot), 'no-icon');
...
describe('loadAllSkills', () => {
  const getWorkspaceSkillsDir = () => getWorkspaceSkillsPath(workspaceRoot);
...
describe('skillExists', () => {
  it('should return true for existing skill with SKILL.md', () => {
    createSkill(getWorkspaceSkillsPath(workspaceRoot), 'exists-skill');
  ...
  it('should return false for directory without SKILL.md', () => {
    createEmptySkillDir(getWorkspaceSkillsPath(workspaceRoot), 'empty');
