...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    ...
    switch (command.type) {
      ...
      case 'respondAutomationConfirmation':
        return sessionManager.respondToAutomationConfirmation(sessionId, command.messageId, command.confirmed)
      case 'addAnnotation':
  ...
  server.handle(RPC_CHANNELS.sessions.GET_PERMISSION_ALLOWANCES, async (
    _ctx,
    sessionId: string
  ) => {
    return sessionManager.getSessionPermissionAllowances(sessionId)
  })
