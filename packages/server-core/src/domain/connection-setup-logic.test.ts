...
describe('createBuiltInConnection — OpenCode Zen/Go templates', () => {
  it('creates opencode-zen with pi_compat, no customEndpoint but seeded catalog models', () => {
    const conn = createBuiltInConnection('opencode-zen')
    expect(conn.slug).toBe('opencode-zen')
    expect(conn.name).toBe('OpenCode Zen')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
    expect(conn.piAuthProvider).toBe('opencode-zen')
  ...
  it('creates opencode-go with pi_compat, no customEndpoint but seeded catalog models', () => {
    const conn = createBuiltInConnection('opencode-go')
    expect(conn.slug).toBe('opencode-go')
    expect(conn.name).toBe('OpenCode Go')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
    expect(conn.piAuthProvider).toBe('opencode-go')
    expect(conn.customEndpoint).toBeUndefined()
    expect(Array.isArray(conn.models)).toBe(true)
    expect(conn.models!.length).toBeGreaterThan(0)
