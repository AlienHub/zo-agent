...
describe('createBuiltInConnection seeds midStreamBehavior', () => {
  ...
  it("Pi API key (Zo Backend) → 'steer'", () => {
...
describe('createBuiltInConnection — OpenCode Zen/Go templates', () => {
  it('creates opencode-zen-anthropic with custom endpoint and catalog models', () => {
    const conn = createBuiltInConnection('opencode-zen-anthropic')
    expect(conn.slug).toBe('opencode-zen-anthropic')
    expect(conn.name).toBe('OpenCode Zen (Claude / Qwen)')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
    expect(conn.piAuthProvider).toBe('opencode-zen-anthropic')
  ...
  it('creates opencode-zen-openai with custom endpoint and catalog models', () => {
    const conn = createBuiltInConnection('opencode-zen-openai')
    expect(conn.slug).toBe('opencode-zen-openai')
    expect(conn.name).toBe('OpenCode Zen (GPT / GLM / Kimi)')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
  ...
  it('creates opencode-go-anthropic with custom endpoint and catalog models', () => {
    const conn = createBuiltInConnection('opencode-go-anthropic')
    expect(conn.slug).toBe('opencode-go-anthropic')
    expect(conn.name).toBe('OpenCode Go (MiniMax / Qwen)')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
  ...
  it('creates opencode-go-openai with custom endpoint and catalog models', () => {
    const conn = createBuiltInConnection('opencode-go-openai')
    expect(conn.slug).toBe('opencode-go-openai')
    expect(conn.name).toBe('OpenCode Go (GLM / Kimi / DeepSeek)')
    expect(conn.providerType).toBe('pi_compat')
    expect(conn.authType).toBe('api_key_with_endpoint')
